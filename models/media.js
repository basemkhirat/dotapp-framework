...
schema.path("thumbnails").get(function () {
    ...
    if (this.image.path) {
        ...
        sizes.forEach((size) => {
            thumbnails[size] = Storage.disk(this.image.storage).url(
                Image.getThumbnailFileName(this.image.path, size)
            );
        });

        thumbnails.default = Storage.disk(this.image.storage).url(
            this.image.path
        );
        ...
        thumbnails.default =
            ...
            "/" +
            "default/files/" +
            path.extname(this.data.path).split(".").pop() +
            ".png";
    } else {
        thumbnails.default =
            Config.get("app.url") +
...
schema.pre("remove", function (next) {
    if (this.image) {
        ...
        sizes.forEach((size) => {
            Storage.disk(this.image.storage).delete(
                Image.getThumbnailFileName(this.image.path, size)
            );
        });
    }
